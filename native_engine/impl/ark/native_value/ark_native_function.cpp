...
ArkNativeFunction::ArkNativeFunction(ArkNativeEngine* engine,
                                     ...
                                     NativeCallback cb,
                                     void* value)
    : ArkNativeFunction(engine, Local<JSValueRef>())
{
    auto vm = const_cast<EcmaVM*>(engine->GetEcmaVm());
    LocalScope scope(vm);

    funcInfo_ = new NativeFunctionInfo();
    funcInfo_->engine = engine;
    funcInfo_->callback = cb;
    funcInfo_->data = value;


    Local<FunctionRef> fn = FunctionRef::New(vm, NativeFunctionCallBack, reinterpret_cast<void*>(funcInfo_));
    ...
    : ArkNativeFunction(engine, Local<JSValueRef>())
{
    ...
    Local<FunctionRef> fn = FunctionRef::NewClassFunction(vm, NativeClassFunctionCallBack, reinterpret_cast<void*>(funcInfo_));
    Local<StringRef> fnName = StringRef::NewFromUtf8(vm, name);
    fn->SetName(vm, fnName);
                                                            ...
                                                            void* data)
{
    ...
    return scope.Escape(ret.ToLocal(vm));
}
